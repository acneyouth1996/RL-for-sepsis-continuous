...
# Initial data manipulations, fill na chartime with chartdate
...
for icustayid in icustayidlist:
    ...
    if len(ab) > 0 and len(bact) > 0:   # If we have data for both: proceed
        ...
        # D in shape # of antibiotics * # of cultures
        ...
        for i in range(D.shape[0]):  #looping through all rows of adminsitered antibiotics, from early to late
            ...
            if M <= 24 and ab1 <= bact1:      # if ab was first and delay < 24h
                onset[icustayid][0] = subj_bact.iloc[0] # subject id
...
Reflabs_values = np.unique(Reflabs.fillna(-10000))[1:] # I guess the default one in this list is the -10000, check on it later
Reflabs_id_dict = {}
...
for icustayid in icustayidlist:
    ...
    if qst > 0:  # if we have a flag
        ...
        if d1[0] > 6574:  # If older than 18 years old
            ...
            # ii is the index of rows (timestamps where fulfil the below requirements)
...
# Estimate GCS from RASS - data from Wesley JAMA 2003 (Glasgow coma scale)
...
# NO FiO2, NO O2 flow, no interface OR cannula 不是很懂，需要问问专业人士
...
# NO FiO2, YES O2 flow, face mask OR.... OR ventilator (assume it's face mask) 和上面一样
...
colnorm = ['age', 'Weight_kg', 'GCS', 'HR', 'SysBP', 'MeanBP', 'DiaBP', 'RR', 'Temp_C',\
        'FiO2_1', 'Potassium', 'Sodium', 'Chloride', 'Glucose', 'Magnesium', 'Calcium', 'Hb', \
...
vc_raw = copy.deepcopy(vc)
vc_raw = vc_raw.reshape(-1, 1)
print(vc_raw.shape)
