...
class ModelContainer(AbstractContainer):
    ...
    def make_encoder(self, hidden_size, state_dim, num_actions, context_dim):
        self.gen = baseRNN_generate(hidden_size, state_dim, num_actions,  context_dim).to(self.device)
        return self.gen
...
class baseRNN_generate(nn.Module):
    def __init__(self, h_size, obs_dim, num_actions,  context_dim=0):
...
class baseRNN_predict(nn.Module):
    def __init__(self, h_size, obs_dim, num_actions):
